...
for(m in model)
{
  for(s in selfing)
  {
    for(q in qtls)
    {
      untar(tarfile = paste("Sim2_Model",m,"_",s,"self_",q,"QTLs_fitperpop/neutral.tar.gz",sep=""),compressed = "gzip",exdir = paste("Sim2_Model",m,"_",s,"self_",q,"QTLs_fitperpop",sep=""))
      for(f in fecundity)
      {
        for(w in selection)
        {
          for(c in cas)
          {
            for(r in repetition)
            {
              if(file.exists(paste("VarCompForFSTs_neutral/var_comp_model",m,"_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,"_neutral.rda",sep=""))==FALSE)  
              {
                temp <- read.fstat(paste("Sim2_Model",m,"_",s,"self_",q,"QTLs_fitperpop/neutral/Sim2_Model",m,"_",s,"self_",q,"QTLs_Cas",c,"_SelInt",w,"_fec",f,"_200_",r,".dat",sep=""))
                ...
                if (unique(is.na(temp$Population)==FALSE))
                {
                  ...
                  save(file=paste("VarCompForFSTs_neutral/var_comp_model",m,"_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,"_neutral.rda",sep=""),var_comp) 
                }
      ...
      unlink(paste("Sim2_Model",m,"_",s,"self_",q,"QTLs_fitperpop/neutral",sep=""),recursive = TRUE)
    }
  }
}
...
for(m in model)
{
  for(s in selfing)
  {
    for(q in qtls)
    {
      untar(tarfile = paste("Sim2_Model",m,"nul_",s,"self_",q,"QTLs_fitperpop/neutral.tar.gz",sep=""),compressed = "gzip",exdir = paste("Sim2_Model",m,"nul_",s,"self_",q,"QTLs_fitperpop",sep=""))
      for(f in fecundity)
      {
        for(w in selection)
        {
          for(c in cas)
          {
            for(r in repetition)
            {
  if(file.exists(paste("VarCompForFSTsNul_neutral/var_comp_model",m,"nul_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,"_neutral.rda",sep=""))==FALSE)  
{
              temp <- read.fstat(paste("Sim2_Model",m,"nul_",s,"self_",q,"QTLs_fitperpop/neutral/Sim2_Model",m,"nul_",s,"self_",q,"QTLs_Cas",c,"_SelInt",w,"_fec",f,"_200_",r,".dat",sep=""))
              #Définition des noms de paramètres
           ...
           if (unique(is.na(temp$Population)==FALSE))
              {
                ...
                save(file=paste("VarCompForFSTsNul_neutral/var_comp_model",m,"nul_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,"_neutral.rda",sep=""),var_comp) 
              }
                 Model <- m ; Selfing <- s ; QTLs <- q ; Fecundity <- f ; Selection <- w ; Cas <- c ; Repetition <- r
                #et on fait du ménage
                 rm(list = c("var_comp","Model", "Selfing", "QTLs", "Fecundity", "Selection", "Cas", "Repetition", "Environment", "Population","temp"))
      ...
      unlink(paste("Sim2_Model",m,"nul_",s,"self_",q,"QTLs_fitperpop/neutral",sep=""),recursive = TRUE)
    }
  }
}
...
for(s in selfing)
{
  for(q in qtls)
  {
    untar(tarfile = "INIT2.tar.gz",files = paste("INIT2/init2_carto_",s,"self_",q,"QTLs/neutral/init2_carto_",s,"self_",q,"QTLs.bin_200000_1.dat",sep=""),compressed = "gzip",exdir = "INIT2")
    temp <- read.fstat(paste("INIT2/INIT2/init2_carto_",s,"self_",q,"QTLs/neutral/init2_carto_",s,"self_",q,"QTLs.bin_200000_1.sat",sep=""))
    ...
    if (unique(is.na(temp$Population)==FALSE))
    {
      setwd("~/nemo_cluster/Sims2")
      var_comp<-varcomp.glob(data.frame(temp$Population,temp$pop),(temp[,-c(1,10)])) ;
      save(file=paste("VarCompForFSTsEq_neutral/var_comp_equilibrium_",s,"self_",q,"QTLs_neutral.rda",sep=""),var_comp) 
    }
    Model <- m ; Selfing <- s ; QTLs <- q ; Fecundity <- f ; Selection <- w ; Cas <- c ; Repetition <- r
    #et on fait du ménage
