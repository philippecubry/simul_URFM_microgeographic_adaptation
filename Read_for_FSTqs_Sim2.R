          ...
          for(c in cas)
          {
            for(r in repetition)
            {
              ...
              if (unique(is.na(temp$Population)==FALSE))
              {
                ...
                save(file=paste("VarCompForFSTqs/var_comp_model",m,"_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,".rda",sep=""),var_comp) 
              }
          ...
          for(c in cas)
          {
            for(r in repetition)
            {
              ...
              if (unique(is.na(temp$Population)==FALSE))
              {
                ...
                save(file=paste("VarCompForFSTqsNul/var_comp_model",m,"nul_cas",c,"_",f,"fec_",s,"self_selection",w,"_",q,"QTLs_rep",r,".rda",sep=""),var_comp) 
              }
              Model <- m ; Selfing <- s ; QTLs <- q ; Fecundity <- f ; Selection <- w ; Cas <- c ; Repetition <- r
              #et on fait du ménage
              rm(list = c("var_comp","Model", "Selfing", "QTLs", "Fecundity", "Selection", "Cas", "Repetition", "Environment", "Population","temp","v"))
