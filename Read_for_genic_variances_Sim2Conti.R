...
#On teste si toutes les pops sont occupées
          ifelse(length(unique(temp$pop))==24,
                ...
                {Genic_VarPopulation <- 0
                 ...
                 #On sauve cet objet
                write.table(x = res_temp_var,file = paste("Genic_variances_components/Model",m,"_conti_Selfing",s,"_QTLs",q,"_Fecundity",f,"_Selection",w,"_Cas",c,"_repetition",r,".txt",sep=""))
...
#On sauve le tout
write.table(x = results,file = paste("Genic_variances_components/main/normal_conti",sep=""))
        ...
        for(w in selection) {
          for(c in cas) {
            for(r in repetition){
              ...
              #Définition des noms de paramètres
              Model <- rep(paste(m,"conti_nul"),length(temp[,1])) ; Selfing <- rep(s,length(temp[,1])) ; QTLs <- rep(q,length(temp[,1])) ; Fecundity <- rep(f,length(temp[,1]))
              Selection <- rep(w,length(temp[,1])) ; Cas <- rep(c,length(temp[,1])) ; Repetition <- rep(r,length(temp[,1])) ; Environment <- rep(NA,length(temp[,1]))
              Population <- rep(NA,length(temp[,1]))
              temp_var<-NULL
              #On ajoute les valeurs des paramètres à temp
...
#On sauve cet objet
write.table(x = res_temp_var,file = paste("Genic_variances_components/Model",m,"_conti_nul_Selfing",s,"_QTLs",q,"_Fecundity",f,"_Selection",w,"_Cas",c,"_repetition",r,".txt",sep=""))
