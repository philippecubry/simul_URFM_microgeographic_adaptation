...
#On transforme certaines variables en facteurs, on réordonne les cas 1, 2, 3, 4 en 2, 1, 3, 4 et les sélections en 50,20,5,1
results$replicate<-as.factor(results$replicate)
results$Case<-factor(results$Cas,levels=c("2","1","3","4")) ; levels(results$Case) <- c("A","B","C","D")
...
g<-ggplot(results)
g<- g + aes(Selection,(adlt.nbr),shape = QTLs,colour=QTLs) + facet_grid(Fecundity ~ Case+Selfing,labeller = label_both) + scale_y_continuous("Number of adults in the meta-population") + geom_jitter() + scale_shape(solid=FALSE) + scale_colour_hue(h=c(50,900))
pdf(file = "Demography for biallelic mutation model and heterogeneous populations", height = 12, width = 16, paper = "special")
print(g)
dev.off()  
