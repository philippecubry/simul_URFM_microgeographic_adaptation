...
#On teste si toutes les pops sont occupées
          if(length(unique(temp$pop))==24) {
                 ...
                 temp$E <- temp$P1 - temp$G1
                 for (j in 1:24){ heritabilities_temp[j] <- (var(temp[which(temp$pop == j),]$G1)/(var(temp[which(temp$pop == j),]$G1)+var(temp[which(temp$pop == j),]$E)))}
                 }
    Model <- m ; Selfing <- s ; QTLs <- q ; Fecundity <- f ; Selection <- w ; Cas <- c ; Repetition <- r
    res_temp<-as.data.frame(cbind(Model,Cas,Selfing,QTLs,Fecundity,Selection,Repetition,heritabilities_temp))
    
    heritabilities <- rbind(res_temp,heritabilities)
...
write.table(results, file="QSTs_Sims2.txt")
####
